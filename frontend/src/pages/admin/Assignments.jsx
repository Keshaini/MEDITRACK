...
import { Link as LinkIcon, Plus, Search, Trash2, Eye, ArrowLeft, AlertCircle, Filter } from 'lucide-react';
...
const Assignments = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${API_URL}/admin/assignments/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="px-6 py-3 bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-bold rounded-lg hover:shadow-lg flex items-center space-x-2"
            >
              <Plus size={20} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
            ...
            <div className="bg-green-50 rounded-lg p-4 border-2 border-green-200">
              ...
              <p className="text-3xl font-bold text-green-900">{assignments.filter(a => a.status === 'active').length}</p>
            </div>
            <div className="bg-blue-50 rounded-lg p-4 border-2 border-blue-200">
              <p className="text-sm text-blue-600 font-medium">Doctors Assigned</p>
              <p className="text-3xl font-bold text-blue-900">{new Set(assignments.map(a => a.doctorId)).size}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-md p-6 mb-6">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search by patient name, doctor name, or specialization..."
              value={searchTerm}
          ...
          {filteredAssignments.length > 0 ? (
              ...
              <table className="w-full">
                ...
                <tbody>
                  {filteredAssignments.map((assignment, index) => (
                    <tr key={assignment._id || index} className="border-b border-gray-100 hover:bg-gray-50">
                      <td className="py-4 px-6">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <p className="font-semibold text-gray-900">{assignment.patientName || 'Unknown Patient'}</p>
                            <p className="text-sm text-gray-500">{assignment.patientEmail || 'N/A'}</p>
                          </div>
                        </div>
                      </td>
                      <td className="py-4 px-6">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <p className="font-semibold text-gray-900">Dr. {assignment.doctorName || 'Unknown'}</p>
                            <p className="text-sm text-gray-500">{assignment.doctorEmail || 'N/A'}</p>
                          </div>
                        </div>
                      </td>
                      <td className="py-4 px-6">
            ...
            <div className="text-center py-16">
              ...
              <p className="text-gray-600 mb-6">
                {searchTerm ? 'Try adjusting your search' : 'Start by creating your first doctor-patient assignment'}
              </p>
              <button
                onClick={() => navigate('/admin/assign-doctor')}
                className="px-6 py-3 bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-bold rounded-lg inline-flex items-center space-x-2"
              >
                <Plus size={20} />
